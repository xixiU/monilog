...
@Slf4j
@AllArgsConstructor
public class ApolloChangeListener implements ConfigChangeListener {
    private final Runnable callback;

    public static void register(Runnable callback) {
        try {
            // 使用ApolloConfigChangeListener方法不生效，手动注入一个监听器
            com.ctrip.framework.apollo.ConfigService.getAppConfig().addChangeListener(new ApolloChangeListener(callback));
        } catch (Throwable e) {
