...
class MoniLogAppListener implements ApplicationListener<ApplicationPreparedEvent> {
    ...
    public void onApplicationEvent(ApplicationPreparedEvent event) {
        ...
        try {
            //这里仅增加redisTemplate，不包括Redisson和RedisCacheManager， Redisson的比较复杂，将通过Aop实现，而RedisCacheManger将通过字节码增强实现
    ...
    private void enhanceRedisTemplate(ConfigurableApplicationContext ctx) {
        ...
        for (RedisTemplate t : templates.values()) {
            RedisConnectionFactory proxy = RedisMoniLogInterceptor.buildProxy(t.getConnectionFactory(), t.getKeySerializer(), t.getValueSerializer(), moniLogProperties.getRedis());
